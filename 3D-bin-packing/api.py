...
# set item and box information
@app.route("/setItemAndBox", methods=["POST"])
@cross_origin()
def setItemAndBoxAPI():
    # Crear box (contenedor)
    if flask.request.method == "POST":
        data = flask.request.json

        # Validate required keys
        if "contenedor" not in data or "cajas" not in data:
            return jsonify({"Success": False, "Reason": "Invalid data format"}), 400

        # Create Contenedor object
        contenedor_data = data["contenedor"]
        contenedor = Contenedor(
            id=contenedor_data["id"],
            width=contenedor_data["width"],
            height=contenedor_data["height"],
            depth=contenedor_data["depth"],
        )

        # Create Caja objects
        cajas_data = data["cajas"]
        cajas = [
            Caja(
                id=caja_data["id"],
                width=caja_data["width"],
                height=caja_data["height"],
                depth=caja_data["depth"],
            )
            for caja_data in cajas_data
        ]

        cajas_output = controller(contenedor, cajas)

        return jsonify(cajas_output)
