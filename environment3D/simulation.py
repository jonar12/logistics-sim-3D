...
# os.chdir(os.path.join(os.getcwd(), "environment3D")) # chdir: Se utiliza para cambiar el directorio de trabajo actual a la ruta especificada
    ...
    async with aiohttp.ClientSession() as session:
        for _ in range(100):
            async with session.get(URL_BASE + LOCATION) as response:
                CAJAS.append(await response.json())
...
filenames = ["./textures/acero_negro.png", "./textures/llanta.png", "./textures/acero_amarillo.png", "./textures/caja.png"]
...
def Init():
    ...
    for i in range(nCajas):
        cajas.append(Caja(DimBoard,1,textures,3))
